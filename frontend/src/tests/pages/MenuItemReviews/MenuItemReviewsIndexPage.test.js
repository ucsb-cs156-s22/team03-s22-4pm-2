...
describe("MenuItemReviewsIndexPage tests", () => {
    ...
    test("renders three menuItemReviews without crashing for regular user", async () => {
        ...
        await waitFor(  () => { expect(getByTestId(`${testId}-cell-row-0-col-id`)).toHaveTextContent(36); } );
        expect(getByTestId(`${testId}-cell-row-1-col-id`)).toHaveTextContent(120);
        expect(getByTestId(`${testId}-cell-row-2-col-id`)).toHaveTextContent(34);
    ...
    test("test what happens when you click delete, admin", async () => {
        ...
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/MenuItemReview/all").reply(200, menuItemReviewsFixtures.threeMenuItemReviews);
        axiosMock.onDelete("/api/MenuItemReview").reply(200, "MenuItemReview with id 36 was deleted");
